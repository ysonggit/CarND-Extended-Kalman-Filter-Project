...
void FusionEKF::ProcessMeasurement(const MeasurementPackage &measurement_pack) {
  ...
  if (!is_initialized_) {
    ...
    if (measurement_pack.sensor_type_ == MeasurementPackage::RADAR) {
      ...
      /**
         Initialize state. (only update x and y)
       */
  ...
  // update process noise and covariance
  float noise_ax = 9.0;
  float noise_ay = 9.0;
  ...
  ekf_.Q_ = MatrixXd(4, 4); // initial matrix is all 0s
  ekf_.Q_ << 0.25 * noise_ax * dt_4, 0, 0.5 * noise_ax * dt_3, 0, 0,
      noise_ay * dt_4 * 0.25, 0, noise_ay * dt_3 * 0.5, noise_ax * dt_3 * 0.5,
      0, noise_ax * dt_2, 0, 0, noise_ay * dt_3 * 0.5, 0, noise_ay * dt_2;
  ...
  if (measurement_pack.sensor_type_ == MeasurementPackage::RADAR) {
    ...
    // Radar updates EKF (nonlinear model)
    ekf_.H_ = tools.CalculateJacobian(ekf_.x_);
    ekf_.R_ = R_radar_;
    ekf_.UpdateEKF(measurement_pack.raw_measurements_);
  } else {
    // Laser updates KF (linear model)
    // cout<<"------------------update KF-------------------"<<endl;
    ekf_.H_ = H_laser_;
    ekf_.R_ = R_laser_;
